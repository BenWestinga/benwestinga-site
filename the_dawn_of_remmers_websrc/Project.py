...
def save_db(db):
    # NOTE: On web/Pygbag builds writing into the bundled asset directory may
    # not persist between sessions.  The try/except ensures this never blocks
    # startup or gameplay.
...
async def run_boss(i, screen):
    try:
        ...
        pass
    if i == 0:
        return await Joe.bossfight_Joe(screen)
    if i == 1:
        return await ump.bossfight_ump(screen)
    if i == 2:
        return await Stein.bossfight_Stein(screen)
    if i == 3:
        return await beast.bossfight_beast(screen)
    if i == 4:
        return await Man.bossfight_Man(screen)
    if i == 5:
        return await Did.bossfight_Did(screen)
    if i == 6:
        return await crazy.bossfight_crazy(screen)
    if i == 7:
        return await Hottie.bossfight_Hottie(screen)
    if i == 8:
        return await Bond.bossfight_Bond(screen, start_stage=1)
    return None
