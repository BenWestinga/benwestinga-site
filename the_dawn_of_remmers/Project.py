...
# DB_FILE is unused directly in web if we use localstorage, but kept for desktop
...
def load_db():
    ...
    if sys.platform == "emscripten":
        try:
            val = platform.window.localStorage.getItem("player_data")
            if val:
                d = json.loads(val)
    ...
    if isinstance(ac, list):
        ...
        for raw_name in ac:
            ...
            legacy_accounts[name] = ensure_acc({
                ...
                "settings": {"shield_enabled": not legacy_no_shields},
                "achievements": {}
            })
        ac = legacy_accounts
...
def save_db(db):
    data_str = json.dumps({"accounts": db.get("accounts", {})}, indent=2)
    if sys.platform == "emscripten":
        try:
            platform.window.localStorage.setItem("player_data", data_str)
        except Exception as e:
...
async def run_boss(i, screen):
    pygame.mouse.set_pos(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)
    if i == 0: return await Joe.bossfight_Joe(screen)
    if i == 1: return await ump.bossfight_ump(screen)
    if i == 2: return await Stein.bossfight_Stein(screen)
    if i == 3: return await beast.bossfight_beast(screen)
    if i == 4: return await Man.bossfight_Man(screen)
    if i == 5: return await Did.bossfight_Did(screen)
    if i == 6: return await crazy.bossfight_crazy(screen)
    if i == 7: return await Hottie.bossfight_Hottie(screen)
    if i == 8: return await Bond.bossfight_Bond(screen, start_stage=1)
    return None
