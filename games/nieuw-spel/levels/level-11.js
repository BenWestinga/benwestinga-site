import grassGoon from "../enemies/enemy-09.js";
import knight from "../enemies/enemy-10.js";
import camoGoon from "../enemies/enemy-11.js";
import shotgunGoon from "../enemies/enemy-12.js";
import snake from "../enemies/enemy-13.js";

export const config = {

    number: 11,

    name: "Level 11",

    startDelayMs: 5000,

    background: {
        image: "grass.png",
        alpha: 0.48,
        color: "#6f9f4d"
    },

    enemyTypes: {
        grassGoon,
        knight,
        camoGoon,
        shotgunGoon,
        snake
    },

    spawnGroups: [

        { enemy: "grassGoon", count: 5, start: 5, duration: 50},
        { enemy: "knight", count: 5, start: 5, duration: 50},
        { enemy: "camoGoon", count: 5, start: 5, duration: 50},
        { enemy: "shotgunGoon", count: 5, start: 5, duration: 50},
        { enemy: "snake", count: 5, start: 5, duration: 50},
    ]
};


export async function start(context = {}) {
    return window.startStoryLevel(
        config,
        context
    );
}


export async function startLevel(context = {}) {
    return start(
        context
    );
}


export default {
    config,
    start,
    startLevel
};